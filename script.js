...
function handleCheckout() {
    // >>> LOG DE DIAGNÓSTICO: INÍCIO DO CHECKOUT
    console.log("[DEBUG CHECKOUT] Iniciando checkout."); 
    
    if (!currentUser) {
        console.log("[DEBUG CHECKOUT] Bloqueado: Usuário não logado.");
        showToast("Você precisa fazer login para finalizar a compra.", "danger");
        openModal("loginModal");
        return;
    }
    if (userCart.length === 0) {
        console.log("[DEBUG CHECKOUT] Bloqueado: Carrinho vazio.");
        showToast("Seu carrinho está vazio.", "info");
        return;
    }

    // >>> LOG DE DIAGNÓSTICO: PRÉ-REQUISITOS ATENDIDOS
    console.log("[DEBUG CHECKOUT] Pré-requisitos atendidos. Abrindo modal de confirmação."); 
...
function setAuthDependentUI(user) {
  ...
  if (checkoutBtn) {
    ...
    // >>> LOG DE DIAGNÓSTICO: ESTADO DO BOTÃO DE CHECKOUT
    console.log(`[DEBUG UI] Estado do Checkout: Logado: ${userLoggedIn}, Itens no Carrinho: ${userCart.length}`);
